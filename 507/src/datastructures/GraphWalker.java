...
public abstract class GraphWalker<V, E> {
	...
	/*
	 * So far the recursive solution is more elegant. Better leave this turned
	 * off.
	 */
	 ...
	 * 
	 * @param edge
	 *            The edge that may be traversed.
	 ...
	 * 
	 * @param node
	 *            The node that has been discovered.
	 */
	 ...
	 * 
	 * @param node
	 *            The node that is left.
	 */
	public abstract void finish(V node);

	/*
	 * A non-recursive solution of the dfs iteration. Not sure this really
	 * works...
	 */
		...
		while (current != null) {
			...
			if (!visited.contains(current)) {
				...
				for (E succEdge : graph.outgoingEdgesOf(current)) {
					...
					// there may be a problem iff the successor can be the same
					// node twice (or more)
					// think of succ normal flow + succ exception flow. But this
					// never happens in the current
					// code. Even with empty catch blocks.
					final V succ = graph.getEdgeTarget(succEdge);
					if (!visited.contains(succ)) {

						// XXX this is slow and should be removed with some
						// elegant solution
