...
public class DominatorTreeBuilder2<V, E> {
    ...
    public DirectedAcyclicGraph<V,DefaultEdge> getDominationTree() {
    ...
    private void compute() {
        ...
        final HashMap<Integer,BitSet> bucket = new HashMap<Integer,BitSet>();
    ...
    private final class DomDFSNumWalker extends GraphWalker<V, E> {
         ...
         */
        final HashMap<V, Integer> semi = new HashMap<V,Integer>();
        /*
         * Stores the parent node of each node during dfs computation. Nodes are identified
         * by their dfs number.
         */
        final HashMap<V, Integer> parent = new HashMap<V,Integer>();
        /*
         * Remembers the dfsnumber of the predecessor in the dfs traversal.
         */
        private final Stack<Integer> pred = new Stack<Integer>();
    ...
    private static final class Forest<V> extends SimpleDirectedGraph<V, ForestEdge> {
        ...
        private final HashMap<V,Integer> semi;

        public Forest(HashMap<V,Integer> semi) {
    ...
    public final boolean assertConsistence() {
        DirectedAcyclicGraph<V,DefaultEdge> domTree = getDominationTree();

        boolean consistent = true;

        for (V node : domTree.vertexSet()) {
            Set<V> treeDom = new HashSet<V>();
            for (DefaultEdge edge : domTree.outgoingEdgesOf(node)) {
